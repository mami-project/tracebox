...
static void probe_print(scamper_probe_t *probe)
{
  ...
  if(probe->pr_ip_dst->type == SCAMPER_ADDR_TYPE_IPV4)
	...
	return;
      ...
      if((probe->pr_ip_off & IP_OFFMASK) != 0 || (probe->pr_no_trans && probe->pr_ip_proto != IPPROTO_TCP))
	{
	...
	return;
      ...
      if(probe->pr_ip_off != 0 || (probe->pr_no_trans && probe->pr_ip_proto != IPPROTO_TCP))
	{
...
static probe_state_t *probe_state_alloc(scamper_probe_t *pr)
{
  ...
  if(SCAMPER_ADDR_TYPE_IS_IPV4(pr->pr_ip_dst))
    {
      if((pr->pr_ip_off & IP_OFFMASK) != 0 || (pr->pr_no_trans && pr->pr_ip_proto != IPPROTO_TCP))
    ...
    {
      if(pr->pr_ip_off != 0 || (pr->pr_no_trans && pr->pr_ip_proto != IPPROTO_TCP))

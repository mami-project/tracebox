...
static void dump_tracebox(scamper_tracebox_t *tracebox)
{
  ...
  uint32_t i;
  uint16_t len, u16, datalen;
  uint8_t proto, flags, iphlen, tcphlen, u8, *tmp, v;
  uint32_t seq, ack, server_isn, client_isn, off, id, u32;
  char src[64], dst[64], buf[128], ipid[12];
  size_t soff;
  int j, frag;

  ipid[0] = '\0';
  
  printf("tracebox %s mode from %s to %s\n", scamper_tracebox_modes[tracebox->printmode],
  scamper_addr_tostr(tracebox->src, src, sizeof(src)),
  scamper_addr_tostr(tracebox->dst, dst, sizeof(dst)));

  dump_timeval("start", &tracebox->start);
  dump_list_summary(tracebox->list);
  ...
  for(i=0; i<tracebox->pktc; i++)
    {
	  ...
	  iphlen = (pkt->data[0] & 0xf) * 4;
	  len = bytes_ntohs(pkt->data+2);
	  proto = pkt->data[9]; v = 4;

     } else if((pkt->data[0] >> 4) == 6)
        {
	  iphlen = 40;
	  len = bytes_ntohs(pkt->data+4) + iphlen;
	  proto = pkt->data[6]; v = 6;
        }
      else
	{
	  continue;
	}
      timeval_diff_tv(&diff, &tracebox->start, &pkt->tv);
      printf(" [%3d.%03d] %s", (int)diff.tv_sec, (int)(diff.tv_usec / 1000),
	     pkt->dir == SCAMPER_TRACEBOX_PKT_DIR_TX ? "TX\n" : "RX\n");


      if(proto == IPPROTO_TCP)
        {
        pprint_packet(v, proto, SCAMPER_TRACEBOX_ANSWER_FULL, pkt->data);
	    tcphlen = ((pkt->data[iphlen+12] & 0xf0) >> 4) * 4;
        }
      ...
      } else if(proto == IPPROTO_ICMP)
        {
          ...
          printf("   ICMP::TYPE: %x\n",icmp_type);
          printf("   ICMP::CODE: %x\n",icmp_code);
          uint8_t ip_start    = iphlen+8;
          uint8_t trans_start = ip_start+20;
          uint8_t dlen        = len-ip_start;
          uint8_t type;
          ...
          if (icmp_type == 11 && icmp_code == 0) {
            ...
            pprint_packet(v, tracebox->udp ? IPPROTO_UDP : IPPROTO_TCP, type, &(pkt->data[ip_start]));
          }
        }
      else if(proto == IPPROTO_ICMPV6)
        {
          uint8_t icmp_type = pkt->data[iphlen];
          uint8_t icmp_code = pkt->data[iphlen+1];
          printf("   ICMPv6::TYPE: %x\n",icmp_type);
          printf("   ICMPv6::CODE: %x\n",icmp_code);
          uint8_t ip_start    = iphlen+8;
          uint8_t trans_start = ip_start+40;
          uint8_t dlen        = len-ip_start;
          uint8_t type;

         if (icmp_type == 3 && icmp_code == 0) {
            if (len-ip_start <= 0) {
              type = SCAMPER_TRACEBOX_ANSWER_EMPTY;
            }else if (len-trans_start <= 0) {
              type = SCAMPER_TRACEBOX_ANSWER_ONLY_L3;
  ...
  for(f=0; f<argc; f++)
      ...
      while(scamper_file_read(file, filter, &type, &data) == 0)
	{
	  ...
	  switch(type)
        ...
        case SCAMPER_FILE_OBJ_TRACEBOX:
	      dump_tracebox(data);
	      break;
